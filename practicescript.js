...
const { filmAdaptation: hasFilmAdapatation = false } = books[1];
console.log(books[1]);
...
/* B) This time you'll write a function utilizing the power of rest parameters.
      ...
      ...
     */
// access my favorite books (title) as the first element in the object, then next should be a list of books(list)
// for loop should loop through a function to be able to input the title, then any list item we want as long as the length of the array is happening.
...
// We need the function to be able to look through the books array and return a title in Spanish or return no data available if there is no title in spanish.
